...
//                        addObject(android);
                        ...
                        for (int i = 0; i < 3; i++) {
                            Object3DData android = Object3DBuilder.loadV5(parent, Uri.parse("assets://assets/models/ziche.obj"));
                            InputStream open1 = ContentUtils.getInputStream(Uri.parse("assets://assets/models/" + android.getTextureFile()));

                            android.setTextureData(IOUtils.read(open1));
                            android.centerAndScale(2.0f);
                            android.setPosition(new float[]{0f, 0f, i});
                            android.setColor(new float[]{1.0f, 1.0f, 1f, 1.0f});
                            //android.setDrawMode(GLES20.GL_TRIANGLE_FAN);
                            addObject(android);
                        }
                    } catch (Exception ex) {
                        errors.add(ex);
