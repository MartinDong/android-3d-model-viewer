...
public class Object3DData {
    ...
    /**
     * The directory where the files reside so we can build referenced files in the model like material and textures files
     * 文件所在的目录，以便我们可以在模型中构建引用文件，如材质和纹理文件
     */
    private Uri uri;
    /**
     * The assets directory where the files reside so we can build referenced files in the model like material and  textures files
     * 文件所在的资源目录，以便我们可以在模型中构建引用文件，如材质和纹理文件
     */
    ...
    // TODO:rotateM需要重构才能在没有旋转时不调用此函数，
    // TODO：目前，转换每调用一次，就构造一次模型矩阵
    // TODO:这是一种低效的方法，很容易出错
    ...
    public Object3DData(FloatBuffer verts, FloatBuffer normals, ArrayList<Tuple3> texCoords, Faces faces,
        ...
        this.faces = faces;  // parameter "faces" could be null in case of async loading//在异步加载的情况下，参数“faces”可以为空
    ...
    public Object3DData setRotation(float[] rotation) {
        ...
        if (rotation.length == 4) {
            ...
            float z = rotation[2];
            float w = rotation[3];
            // TODO: need deal with 0 degree rotation  angle
            // TODO:需要处理0度旋转角度
     ...
     * 如果OpenGL不支持对GLDrainElements（）使用GL_UNSIGNED_INT，则使用此缓冲区
     *
    ...
    public void centerScale()
         ...
         * 定位模型，使其中心位于原点，并缩放模型，使其最长尺寸不大于maxSize。
         */ {
        // calculate a scale factor
        float scaleFactor = 1.0f;
        float largest = modelDimensions.getLargest();
